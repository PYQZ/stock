    ...
    <div class="form" :style="'width:'+width+'%;padding: 32px 32px 32px 0;background:#fff;'">
        <el-form ref="form" :inline="data.inline||false" :model="data.formFields" :rules="data.rules" :label-width="data.labelWidth||'100px'" :label-position="data.labelPosition||'right'" class="demo-ruleForm">
            ...
            <el-form-item v-for="(item,index) in data.formLabel" :key="index" :label="item.title" :prop="item.prop" :style="item.formItemStyle||''">
                ...
                <el-input v-if="item.type==='input'" v-model="data.formFields[item.prop]" @change="item.change&&item.change(data.formFields[item.prop])" :show-password="item.password" :size="item.size" :disabled="item.disabled" :style="item.style||''" :prefix-icon="item.prefixIcon||''" :maxlength="item.maxlength||''" :placeholder="item.placeholder" show-word-limit></el-input>
                ...
                <el-input v-if="item.type==='textarea'" type="textarea" v-model="data.formFields[item.prop]" @change="item.change&&item.change(data.formFields[item.prop])" :rows="item.rows || 2" :disabled="item.disabled" :resize="item.resize||'none'" :style="item.style||''" :maxlength="item.maxlength" show-word-limit :placeholder="item.placeholder||'请输入内容'"></el-input>
                <el-select v-if="item.type==='select'" v-model="data.formFields[item.prop]" @change="item.change&&item.change(data.formFields[item.prop])" :size="item.size" :disabled="item.disabled" :style="item.style||''" placeholder="请选择">
                    <el-option v-if="item.default&&item.default.show" label="请选择" :value="item.default&&item.default.value"></el-option>
                    ...
                    <el-option v-for="(subItem,key) in item.options" :key="key" :label="subItem[item.key||'label']" :value="subItem[item.value||'value']" :disabled="(item.parent?(data.formFields[item.value||'value']!=subItem[item.value||'value'])&&(item.parent&&data.formFields[item.parent]!=subItem[item.value||'value']):data.formFields[item.prop||'value']!=subItem[item.value||'value'])?false:true">
