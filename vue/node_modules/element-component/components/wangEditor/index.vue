    ...
    methods: {
      setEditor: function () {
        ...
        if (this.emotions.length > 0){
          this.editor.config.emotions = this.emotions;
        }
        if (this.lineHeights.length > 0){
          this.editor.config.lineHeights = this.lineHeights;
        }
        if (this.fontNames.length > 0){
          this.editor.config.fontNames = this.fontNames;
        }
        if (this.colors.length > 0){
          this.editor.config.colors = this.colors;
        }
        if (JSON.stringify(this.fontSizes) != "{}"){
          this.editor.config.fontSizes = this.fontSizes;
        }
        if (JSON.stringify(this.highlight) != "{}"){
          this.editor.config.highlight = this.highlight;
        }
        ...
        this.editor.config.uploadVideoHooks = {
          before: function (xhr) {
            return _this.$emit('beforeVideo', xhr, {
              prevent: true,
              msg: '需要提示给用户的错误信息'
            }, _this.params, _this.editor, _this.$refs.editor);
          },
          fail: function (xhr, editor, result) {
